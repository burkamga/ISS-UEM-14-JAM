...
public class COConsulta {
   ...
   /* public static boolean buscaConsulta(String dataConsulta, int horaConulta){
        List<Vodataconsulta> ConsultaDia = CRUD_Vodataconsulta.listar();
        for (Vodataconsulta Consulta : ConsultaDia){
            if (Consulta.getData().equals(dataConsulta)){
                Voconsultas consulta = CRUD_Voconsultas.buscarConsulta(CRUD_Vodataconsulta.getHoraId(horaConulta, Consulta));
             }
         }
